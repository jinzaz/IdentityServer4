...
namespace NetCore.Identity.UserApiService.Extension
{
    public static class UserIdentityExtension
    {
        ...
        {
            return @this?.Claims?.FirstOrDefault(oo => oo.Type == ClaimTypes.Name.ToString())?.Value;
        }
        ...
        public static string Audience(this ClaimsPrincipal @this)
        {
            return @this?.Claims?.FirstOrDefault(oo => oo.Type == JwtClaimTypes.Audience.ToString())?.Value;
        }
    }
}
